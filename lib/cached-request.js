...
CachedRequest.prototype.cachedRequest = function () {
  ...
  function makeRequest () {
    // call the callback once the metadata and response files have been written
    ...
    if (callback){
      ...
      args[args.length - 1] = function callbackWrap(){
        var args = arguments;
        if (cacheEnd) return callback.apply(null, args); 
    ...
    var request = self.request.apply(null, args);
    requestMiddleware.use(request, cacheEvents);
    request.on("response", function (response) {
      ...
      responseWriter.on('finish', function(){
        writeFileAtomic(self.cacheDirectory + key + ".json", JSON.stringify(meta), function (error) {
